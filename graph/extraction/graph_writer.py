...
class GraphWriter:
    ...
    def convert_to_graph_document(self, chunk_id: str, input_text: str, result: str) -> GraphDocument:
        ...
        # 修复正则表达式以正确处理多行文本和引号
        # 支持多种引号格式和换行
        node_pattern = re.compile(r'\(["\']?entity["\']?\s*:\s*["\']?([^"\'():]+)["\']?\s*:\s*["\']?([^"\'():]+)["\']?\s*:\s*["\']?([^"\']*)["\']?\s*\)', re.MULTILINE | re.DOTALL)
        relationship_pattern = re.compile(r'\(["\']?relationship["\']?\s*:\s*["\']?([^"\'():]+)["\']?\s*:\s*["\']?([^"\'():]+)["\']?\s*:\s*["\']?([^"\'():]+)["\']?\s*:\s*["\']?([^"\']*)["\']?\s*:\s*([\d.]+)\s*\)', re.MULTILINE | re.DOTALL)
            ...
            for match in node_pattern.findall(result):
                ...
                if node_id in self.node_cache:
                    ...
                    new_node = Node(
                        id=node_id,
                        type=node_type,
                        properties={'name': node_id, 'description': description}
                    )
                ...
                if source_id not in nodes:
                    if source_id in self.node_cache:
                        ...
                        new_node = Node(
                            id=source_id,
                            type="未知",
                            properties={'name': source_id, 'description': 'No additional data'}
                        )
                ...
                if target_id not in nodes:
                    if target_id in self.node_cache:
                        ...
                        new_node = Node(
                            id=target_id,
                            type="未知",
                            properties={'name': target_id, 'description': 'No additional data'}
                        )
    ...
    def process_and_write_graph_documents(self, file_contents: List) -> None:
        ...
        Args:
            file_contents: 文件内容列表，每个元素为(filename, filetype, chunks, extraction_results)
        """
        all_graph_documents = []
        all_chunk_ids = []

        # 预分配列表大小
        total_chunks = sum(len(file_content[2]) for file_content in file_contents)  # chunks在索引2的位置
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for file_content in file_contents:
                ...
                filetype = file_content[1]
                chunks = file_content[2]  # chunks在索引2的位置
                extraction_results = file_content[3]  # 提取结果在索引3的位置

                for i, (chunk, result) in enumerate(zip(chunks, extraction_results)):
