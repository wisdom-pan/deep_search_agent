...
class BaseAgent(ABC):
    ...
    def _check_all_caches(self, query: str, thread_id: str = "default"):
        ...
        # 防御性检查：确保缓存管理器是有效对象
        if not hasattr(self, 'global_cache_manager') or not hasattr(self.global_cache_manager, 'get'):
            return None
        ...
        if global_result:
            ...
            # 确保返回字符串类型
            if not isinstance(global_result, str):
        ...
        if fast_result:
            ...
            # 确保返回字符串类型
            if not isinstance(fast_result, str):
