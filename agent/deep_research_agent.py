...
from search.tool.enhanced_deep_research_tool import EnhancedDeepResearchTool 
...
class DeepResearchAgent(BaseAgent):
    ...
    def __init__(self, use_deeper_tool=True, use_enhanced_tool=True):
        """
        初始化增强版深度研究Agent

        Args:
            use_deeper_tool: 是否使用DeeperResearchTool（已有功能）
            use_enhanced_tool: 是否使用增强研究工具（新增：图表、公式、进度）
        """
        # 初始化研究工具
        self.use_deeper_tool = use_deeper_tool
        self.use_enhanced_tool = use_enhanced_tool

        # 优先使用增强版研究工具
        if use_enhanced_tool:
            try:
                self.research_tool = EnhancedDeepResearchTool()
                print("✅ 已加载增强版深度研究工具（支持图表、公式、进度）")

                # 增强版工具使用thinking_stream_enhanced
                self.stream_tool = self.research_tool.thinking_stream_enhanced

                # 加载额外工具（如果DeeperResearchTool也启用）
                if use_deeper_tool:
                    try:
                        deeper_tool = DeeperResearchTool()
                        self.exploration_tool = deeper_tool.get_exploration_tool()
                        self.reasoning_analysis_tool = deeper_tool.get_reasoning_analysis_tool()
                        print("✅ 已加载额外探索和分析工具")
                    except Exception as e:
                        print(f"⚠️ 加载额外工具失败: {e}")

            except Exception as e:
                print(f"❌ 加载增强版研究工具失败: {e}，将使用标准版")
                self._init_standard_tool()

        elif use_deeper_tool:
            try:
    ...
    def _generate_node(self, state):
        ...
        if global_result:
            ...
            return {"messages": [AIMessage(content=str(global_result))]}
        ...
        if cached_result:
            ...
            return {"messages": [AIMessage(content=str(cached_result))]}
